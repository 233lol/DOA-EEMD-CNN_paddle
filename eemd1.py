...
def eemd_p(case,data,index,endi):
    ...
    print("%s finish"%(case))
names=["case12","case12","case12"]
indexs=[2331,2331+614,2331+614*2]
endi=[2331+614,2331+614*2,4173]
datas=[]
for name in names:
...
for i in range(np.size(names)):
    # print(name)
    proc = multiprocessing.Process(target=eemd_p, args=(names[i],datas[i],indexs[i],endi[i]))
    procs.append(proc)
    proc.start()